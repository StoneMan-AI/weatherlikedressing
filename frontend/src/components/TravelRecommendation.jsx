...
const ItemCard = ({ item, type }) => {
  ...
  // 获取图标（仅clothing类型使用PNG，其他类型使用Emoji）
  const getItemIcon = (itemName, itemType) => {
    // 如果是衣服类型，使用clothingIconMap（支持PNG）
    if (itemType === 'clothing') {
      return getClothingIcon(itemName, 'clothing');
    }
    
    // 其他类型（常备用品、急需用品等）使用原来的Emoji映射
    const iconMap = [
      // 常备用品 - 证件
      ['学生证', '🆔'], ['驾驶证', '🆔'], ['身份证', '🆔'], ['护照', '🛂'],
    ...
    for (const [key, icon] of iconMap) {
      if (itemName.includes(key)) {
        return { emoji: icon, pngPath: null };
      }
    }

    // 默认图标
    return { 
      emoji: itemType === 'essential' ? '🎒' : '📦',
      pngPath: null
    };
  };
  
  const iconInfo = getItemIcon(itemName, type);
  ...
  return (
    <div className={`item-card item-card-${type}`}>
      <div className="item-icon">
        {/* 仅clothing类型且PNG存在时才尝试加载PNG图片 */}
        {type === 'clothing' && iconInfo.pngPath && !imageError ? (
          <img 
